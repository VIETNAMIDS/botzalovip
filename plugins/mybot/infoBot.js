...
export async function infoBot(api, message, groupAdmins) {
  ...
  try {
    ...
    console.error(`Lỗi lấy thông tin bot: ${error.message}`);
    try {
      await sendMessageFromSQL(
        api,
        message,
...
export async function detailBot(api, message, groupAdmins) {
  ...
  try {
    ...
    console.error(`Lỗi lấy thông tin chi tiết bot: ${error.message}`);
    try {
      await sendMessageFromSQL(
        api,
        message,
...
async function getBotUidByIndex(index) {
  try {
    ...
    console.error(`Lỗi lấy bot UID theo số thứ tự ${index}: ${error.message}`);
    return null;
  }
}
