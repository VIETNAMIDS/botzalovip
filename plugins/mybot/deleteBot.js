      ...
      try {
        await sendMessageFromSQL(
          ...
          message,
          {
            success: false,
            message:
              "Hiện tại chỉ có admin bot mẹ mới có quyền xoá/reset bot của bạn.",
          }, 
          true,
          60000
        );
      ...
      try {
        await sendMessageFromSQL(
          ...
          message,
          {
            success: false,
            message:
              `Không thể xóa bot khỏi danh sách. Vui lòng thử lại hoặc liên hệ admin!`,
          },
          true,
          60000
        );
...
async function removeBotFromList(uidFrom, webPort) {
  try {
    ...
    // Lấy cấu hình bot để xóa các tệp liên quan
    const botConfig = createBotConfig(uidFrom, webPort || 3000); // webPort mặc định là 3000 nếu không có

    // Danh sách các tệp cần xóa
    const filesToDelete = [
