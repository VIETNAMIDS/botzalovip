...
const { stopBot } = require("./stopBot.js");
const { startBot } = require("./startBot.js");
const { deleteBot } = require("./deleteBot.js");
const { infoBot, detailBot } = require("./infoBot.js");
const { getGlobalPrefix } = require("../../service-dqt/service.js");
const { createBotListImage } = require("../../utils/canvas/listbots-canvas.js");
const { sendMessageFailed, sendMessageComplete, sendMessageWarning, sendMessageResultRequest } = require("./chat-style-fake.js");
const { MessageType } = require("zlbotdqt");
...
const jsonDataDir = path.join("C:", "Users", "Administrator", "Downloads", "HHH_BOT", "HHH_BOT", "mybot", "json-data");
const adminListPath = path.join("C:", "Users", "Administrator", "Downloads", "HHH_BOT", "HHH_BOT", "assets", "data", "list_admin.json");

// Kiểm tra bot hết hạn định kỳ (chạy mỗi 5 phút)
const CHECK_INTERVAL = 5 * 60 * 1000; // 5 phút tính bằng mili giây
    ...
    try {
      ...
      for (const [botUid, botInfo] of Object.entries(myBots)) {
        ...
        if (expiryAt <= now && botInfo.status === "running") {
          ...
          // Gọi stopBot để xử lý logic dừng bot
          await stopBot(api, modifiedMessage, []);
          // Đảm bảo dừng tiến trình trong PM2
          ...
          // Cập nhật trạng thái bot thành expired
...
// Bắt đầu kiểm tra hết hạn khi module được nạp
...
• Chức năng: Đăng ký/sửa đổi thông tin vào hệ thống Hà Huy Hoàng
• Lưu ý: 
   - Không cần nhập dấu []
...
true
    ...
    // Xử lý lệnh admin bot mẹ với số thứ tự
    if (isMotherBotAdmin && ["del", "rs", "start", "stop", "detail", "info", "active", "restart", "shutdown", "addtime", "subtime", "approve", "reject"].includes(subCommand) && args.length >= 3 && /^\d+$/.test(args[2])) {
      ...
      switch (subCommand) {
        ...
        case "active":
        case "restart":
          // Không kiểm tra trạng thái cho admin bot mẹ
...
➤『${prefix}mybot delete』- Xoá bot khỏi hệ thống Hà Huy Hoàng`;
        try {
...
// Hàm phụ chỉ để gửi danh sách bot (ảnh)
async function handleListBots(api, threadId, type, message) {
  let imagePath;
  try {
    // Kiểm tra threadId
    if (!threadId) {
      console.error("ThreadId không hợp lệ");
      return;
    }

    // Kiểm tra sự tồn tại của file mybots.json
    if (!fs.existsSync(myBotsPath)) {
      try {
        await sendMessageFailed(api, message, "❌ Chưa có bot nào được tạo!", true);
      } catch (err) {
        console.error(`Lỗi khi gửi thông báo thất bại: ${err.message}`);
      }
      return;
    }

    // Đọc và kiểm tra danh sách bot
    let myBots;
    try {
      myBots = JSON.parse(fs.readFileSync(myBotsPath, "utf8"));
    } catch (parseError) {
      console.error(`Lỗi khi phân tích mybots.json: ${parseError.message}`);
    ...
    if (botList.length === 0) {
      try {
        ...
        console.error(`Lỗi khi gửi thông báo danh sách trống: ${err.message}`);
      }
      return;
    }

    // Tạo hình ảnh danh sách bot
    try {
    ...
    // Xóa tệp ảnh tạm (nếu tồn tại)
    if (imagePath && fs.existsSync(imagePath)) {
      try {
        fs.unlinkSync(imagePath);
        console.log(`Đã xóa tệp ảnh tạm: ${imagePath}`);
      } catch (unlinkError) {
        console.error(`Lỗi khi xóa tệp ảnh tạm: ${unlinkError.message}`);
      }
    }
  }
}
...
async function handleApprove(api, botUid, dName, duration, threadId, type, message) {
  try {
    ...
    // Đọc và kiểm tra bot trong myBots
    const myBots = JSON.parse(fs.readFileSync(myBotsPath, "utf8"));
    if (!myBots[botUid]) {
      await sendMessageResultRequest(
        api,
    ...
    // Tạo message object an toàn cho startBot
    const modifiedMessage = {
      threadId: threadId,
      data: {
        uidFrom: botUid,
      ...
      type: MessageType.GroupMessage
      // Không bao gồm messageID để tránh lỗi Invalid quote message trong startBot
    };

    // Gọi startBot trong try-catch để bắt lỗi
    try {
      await startBot(api, modifiedMessage, []);
    } catch (startBotError) {
    ...
    // Gửi thông báo xác nhận đến thread gọi lệnh
    await sendMessageResultRequest(
      api,
      type || MessageType.GroupMessage,
      threadId,
...
async function handleActiveAll(api, uidFrom, dName, threadId, type, message, groupAdmins) {
  try {
    ...
    for (const [botUid, botInfo] of Object.entries(myBots)) {
      const modifiedMessage = {
        threadId: threadId, // Sử dụng threadId của lệnh gốc
        data: { uidFrom: botUid, dName: botInfo.displayName || botInfo.name },
        type: MessageType.GroupMessage // Đảm bảo type là GroupMessage
      };
      try {
        await startBot(api, modifiedMessage, groupAdmins);
        successCount++;
...
async function handleNotify(api, uidFrom, dName, content, threadId, type, message) {
  try {
    // Kiểm tra xem file mybots.json có tồn tại không
    if (!fs.existsSync(myBotsPath)) {
      try {
        const messageOptions = { 
        ...
        if (message && typeof message === 'object' && message.messageID) {
          messageOptions.quote = message; // Trích dẫn tin nhắn gốc nếu có
        }
        await api.sendMessage(messageOptions, threadId, type);
      } catch (err) {
    ...
    // Đọc và phân tích file mybots.json
    const myBots = JSON.parse(fs.readFileSync(myBotsPath, "utf8"));
    let sentUids = new Set(); // Set để theo dõi UID đã gửi, tránh trùng lặp
    let successCount = 0; // Đếm số tin nhắn gửi thành công
    let failCount = 0; // Đếm số tin nhắn gửi thất bại

    // Lặp qua từng bot trong mybots.json
    for (const [botUid, botInfo] of Object.entries(myBots)) {
      // Kiểm tra UID bot có hợp lệ (chỉ chứa số)
      if (!/^\d+$/.test(botUid)) {
      ...
      // Gửi thông báo đến chủ bot (botUid)
      if (!sentUids.has(botUid)) {
        try {
          await api.sendMessage(
            ...
            botUid,
            MessageType.DirectMessage // Gửi tin nhắn trực tiếp
          );
          sentUids.add(botUid);
          successCount++;
      ...
      // Kiểm tra và gửi thông báo đến admin của bot
      ...
      if (fs.existsSync(adminFilePath)) {
        ...
        try {
          ...
          adminList = JSON.parse(adminData);
          if (!Array.isArray(adminList)) {
            adminList = []; // Nếu không phải mảng, gán rỗng
          }
        } catch (parseError) {
    ...
    // Gửi báo cáo kết quả về threadId
    try {
      const messageOptions = {
...
async function handleLoadBots(api, uidFrom, dName, threadId, type, message) {
  try {
    ...
    // Cập nhật file mybots.json để đảm bảo định dạng
    fs.writeFileSync(myBotsPath, JSON.stringify(myBots, null, 2));
